...
4. **Move orchestration selectively:** the implemented Rust supervisor proof owns one safe engine, ephemeral workspace lifecycle, stale cleanup, deadline admission, and redacted telemetry. Benchmark and prove safety before moving scheduler and then memory ownership from C to Rust. Keep C kernels as the correctness/performance baseline.
