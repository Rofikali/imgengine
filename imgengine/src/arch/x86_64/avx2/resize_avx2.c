...
static inline void img_resize_blend8_rgb(
    uint8_t *dst_row,
    const uint8_t *row0,
    const uint8_t *row1,
    ...
    uint32_t wy0)
{
    ...
    for (uint32_t c = 0; c < 3; c++)
    {
        ...
        __m256i p00_v = _mm256_setr_epi32(
            p00[0], p00[1], p00[2], p00[3], p00[4], p00[5], p00[6], p00[7]);
        __m256i p10_v = _mm256_setr_epi32(
            p10[0], p10[1], p10[2], p10[3], p10[4], p10[5], p10[6], p10[7]);
        __m256i p01_v = _mm256_setr_epi32(
            p01[0], p01[1], p01[2], p01[3], p01[4], p01[5], p01[6], p01[7]);
        __m256i p11_v = _mm256_setr_epi32(
            p11[0], p11[1], p11[2], p11[3], p11[4], p11[5], p11[6], p11[7]);

        __m256i top_v = _mm256_srli_epi32(
            _mm256_add_epi32(
                _mm256_add_epi32(
                    _mm256_mullo_epi32(p00_v, wx0_v),
                    _mm256_mullo_epi32(p10_v, fx_v)),
                rounding),
            16);
        __m256i bottom_v = _mm256_srli_epi32(
            _mm256_add_epi32(
                _mm256_add_epi32(
                    _mm256_mullo_epi32(p01_v, wx0_v),
                    _mm256_mullo_epi32(p11_v, fx_v)),
                rounding),
            16);

        __m256i out_v = _mm256_srli_epi32(
            _mm256_add_epi32(
                _mm256_add_epi32(
                    _mm256_mullo_epi32(top_v, wy0_v),
