        ...
        switch (task->kind)
        {
            case IMG_TASK_KIND_PIPELINE:
                ...
                img_pipeline_execute_hot(
                    w->ctx,
                    task->payload.pipeline.pipeline,
            ...
            case IMG_TASK_KIND_RENDER_STAGE:
            {
                ...
                task->status = img_runtime_prepare_render_stage(
                    task->payload.render.engine,
                    ctx,
                    task->payload.render.canvas,
                    task->payload.render.layout,
                    task->payload.render.job,
                    task->payload.render.photo,
